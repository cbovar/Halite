...
public class IntelligentBotV5
{
    ...
    public IntelligentBotV5(Map map, ushort myID)
    {
        this.map = map;
        this.myId = myID;
                    ...
                    if (this.map[x, y].Owner == this.myId)
                    {
                        ...
                        if (this.map[x, y].Strength > 1)
                        {
                            if (!degradedMode || ((frame + x + y) % 4 == 0) || this.map[x, y].Strength == 255)
                            {
                                ...
                                singleNet.Forward(false, volume);
                                move = (Direction)singleNet.GetPrediction();
                            }
                        }
    ...
    public void Load()
    {
        ...
        var net = singleNet as Net;
        if (net != null)
        {
            this.inputWidth = net.Layers[0].InputWidth;
        }

        var fluentNet = singleNet as FluentNet;
        if (fluentNet != null)
        {
...
public class IntelligentBotV4
{
    private readonly Dictionary<int, Location> oponents = new Dictionary<int, Location>();
    ...
    private Map map;
    private Net moveNet;
    private Net directionNet;

    private ushort myId;

    private Random random = new Random();

    public IntelligentBotV4(Map map, ushort myID)
    {
        this.map = map;
        this.myId = myID;
        ...
        try
        {
            ...
            for (ushort x = 0; x < this.map.Width; x++)
            {
                for (ushort y = 0; y < this.map.Height; y++)
                {
                    if (this.firstRound)
                    {
                        if (this.map[x, y].Owner != this.myId && this.map[x, y].Owner != 0)
                        {
                            this.oponents[this.map[x, y].Owner] = new Location { X = x, Y = y };
                        }
                    }
                    ...
                    if (this.map[x, y].Owner == this.myId)
                    {
                        ...
                        if (!degradedMode || ((frame + x + y) % 2 == 0))
                        {
                            var volume = this.map.GetVolume(moveNet.Layers[0].InputWidth, this.myId, x, y);
                            moveNet.Forward(false, volume);
                            shouldMove = moveNet.GetPrediction() == 1;
                        }
                        else
                        {
                            // Perf mode
                        ...
                        if (move == Direction.Still)
                        {
                            if (shouldMove)
                            {
                                var chrono2 = System.Diagnostics.Stopwatch.StartNew();

                                var volume = this.map.GetVolume(directionNet.Layers[0].InputWidth, this.myId, x, y);
                                directionNet.Forward(false, volume);
                                move = (Direction)directionNet.GetPrediction() + 1;

                                elapsedDirection += chrono2.Elapsed.TotalMilliseconds;
                            }
                            else
                            {
    ...
    public void Load()
    {
        this.directionNet = NetExtension.LoadNet($"{this.Prefix}direction{this.Suffix}.dat") as Net;
        this.moveNet = NetExtension.LoadNet($"{this.Prefix}move{this.Suffix}.dat") as Net;
    }
}
